...
The evaluator can inspect [pyproject.toml](../../pyproject.toml) and the
complete [uv.lock](../../uv.lock) directly.

Final cumulative run `de09d92b-ea82-4644-9341-d390cc9785b0` used Git
`e1a3b3a3248f1386118f3c3303e7412859cd72bd`. The estimated useful
allocation was 24 cores for the real-model stage (16 Torch threads plus
8 image loaders); HF allocated 64 logical CPUs. The command runtime was
1,912.079 s and remote elapsed time was 32m34s. Billing cost is not exposed
by the local `orx` evidence interface.
...
The final run exported 21 files totaling 231,920 bytes; all decoded byte
counts and SHA-256 hashes matched. The resulting
[science gate](../../evidence/cumulative_science_gate.json) records all five
verdicts and nine passing checks.
